...
test_cfg = dict(
    rpn=dict(
        ...
        nms_thr=0.7,
        min_bbox_size=0),
    #rcnn=dict(score_thr=1e-3, nms=dict(type='nms', iou_thr=0.5), max_per_img=300))
    rcnn=dict(score_thr=0.01, nms=dict(type='nms', iou_thr=0.5), max_per_img=300))
# dataset settings
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file= data_root + 'CrowdHuman/ImageSets/old/trainval.txt',
        img_prefix=data_root + 'CrowdHuman/',
		pipeline=train_pipeline ),
    val=dict(
        type=dataset_type,
        ann_file= data_root + 'CrowdHuman/ImageSets/old/val.txt',
        img_prefix=data_root + 'CrowdHuman/',
...
log_level = 'INFO'
work_dir = root_dir+'mmdet_models/work_dirs/ga_faster_rcnn_r50_caffe_fpn_1x'
load_from = None
resume_from = None
workflow = [('train', 3)]
